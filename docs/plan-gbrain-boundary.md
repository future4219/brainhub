...
状態: 実装・回帰確認完了（2026-09-07）。計画時の基準コミット: `96d5f7d`。以下は合意した計画で、実施結果を末尾に記す。

## 最優先の方針: GBrainの既存機能を使い、Brainhubの実装を最小限にする

ユーザーの方針: 「できるだけGbrainの既存機能を使うだけにしたいねんな、こっちの実装は最小限にしたい」。

最初に自前処理を減らせるか確認し、残る接続処理だけを内部にまとめる。今の実装をすべて温存して、その上に新しい層をかぶせることを目的にしない。
READMEの「GBrainのCLI/APIでできることを再実装しない」を判断基準にする。

| 担当 | 方針 |
|---|---|
| Markdown保存、gitへの反映、検索、索引 | GBrainの既存操作へ任せる。Brainhubに別の保存・検索エンジンを作らない |
| GBrain内部のClient登録・Source grant・トークン発行・失効 | GBrainの既存APIを使う。Brainhub側で発行方式やgrantの強制処理を再実装しない |
| 利用者、所有者、Membership、公開設定、AIへ任せる範囲 | 現在のBrainhubが持つ判断を維持し、GBrainに渡す権限へ対応付ける |
| HTTP転送・内部資格情報の保持・失敗時の復旧記録 | 現行サービスを接続するために必要な最小限の処理を残す |

調査対象は稼働中のGBrain `v0.46.28.0`。最新コードにだけある機能を前提にせず、更新が必要な案は今回の整理から分ける。
計画追記時に稼働イメージのプログラムを読み、`register-client` / `rescope-client` / `revoke-client` と、上流の操作定義からMCPスキーマを生成する処理の存在を確認した。データや設定は変更していない。

実装前には既存APIで削除・置換できる箇所を短い一覧にし、残す自前処理には「GBrainだけでは満たせない現行要件」を記す。未検証の制約を理由に独自機能を増やさない。
...
| `brain.go` の `createRecords` | 脳・所有者と一緒に接続の準備中レコードを組み立てる | 今回は維持。通常の保存経路から独立しており、トランザクションの組み替えは対象外 |
...
ここでいう「GBrain通信」は責務の呼び名であり、新しい大きなクラスやパッケージを必ず追加するという意味ではない。既存proxyへの移動と不要な窓口の削除で成立するなら、それで終える。
...
受け渡しは既存のサーバー内部contextを使う案を第一候補とし、追加する型や変換を最小限にする。解析済み要求と認可結果を対応させ、handlerとproxyで別々の要求を認識する状態を作らない。
...
### 0. GBrainに任せて削れる処理を先に確かめる

- ページの保存・取得・検索は既存MCP操作を転送する。操作ごとの独自usecaseやラッパーを一律に追加しない。
- grantの変更や失効は既存Admin APIを呼ぶ。独自のgrantエンジンは作らない。
- `mcp_catalog.go` の手書き `put_page` 定義は、上流のツール定義を安全に再利用できるか確認する。ただし取得のために複雑なキャッシュや大量の追加通信が必要なら、短い既存の互換処理を残す。無関係な管理ツールまで公開しない。
- `source_id` の付加・除去は現行のSource選択と上流引数仕様をつなぐ処理として扱う。上流だけで同じ境界を保証できると確認できた場合に限り削る。
- Brainhub用OAuthを上流へそのまま転送するだけで、現在の利用者との対応・同意・Membership変更時の制限が保たれるとは仮定しない。現行UXに必要な対応付けを残す。

この確認で不要になった設計は、後続手順から削除する。GBrain機能を自前で包み直す作業を先に始めない。
...
### 4. 初期登録と復旧は、既存処理を維持する

脳・所有者・接続準備レコードが同じDBトランザクションで保存される現行処理を維持する。
新しい初期登録用の窓口を追加して処理を移すだけの変更は、今回の対象から外す。

GBrainの接続発行はトランザクションの外に保ち、発行失敗時のdegraded状態、起動時再開、明示的な再発行を維持する。
`BrainWriterClientRepository` の保存・復旧操作は必要な内部処理として残す。通常のページ保存・検索がそれらを組み立てる形にはしない。
GBrainの既存機能によってこの管理自体を減らせる根拠が見つかった場合は、トランザクションと復旧の検証を付けた別の小さな変更として扱う。
...
4. 通常のページ処理が、接続の準備中・失敗・復旧の保存手順を組み立てない。脳初期登録の既存トランザクションは維持する。
5. 現行の成功・拒否・復旧・ストリーミング動作のテストが通る。
6. GBrainが実装済みの保存・検索・grant管理を再実装していない。残る自前処理の必要性と、削除・統合した処理を説明できる。

計画作成時はコードや稼働環境を変更せず、ユーザーの実装指示を受けて以下を実施した。


## 実施結果（2026-09-07）

- `GBrainToken`、`StripSource`、上位interfaceの内部token取得を削除。MCP usecaseからwriterサービスとreader資格情報repositoryへの依存を除いた。
- handlerのGBrain引数変換と認証ヘッダー設定を既存proxyへ移し、認可済み要求との対応を境界で確認する。新しいgateway、保存エンジン、依存ライブラリは追加していない。
- 接続表示を状態・理由に限定し、起動時照合を `ConnectedUsers` / `Prepare` に変更。初期登録のトランザクションと復旧処理は維持した。
- 不要になったrepository mockとtoken取得用のダミーメソッドを削除。鍵の呼び出し位置を前提にしたテストは、実際のhandler→proxy→模擬上流での認可・転送確認へ移した。
- GBrainの既存保存・検索・Admin APIを引き続き使用。残す保存先変換と小さいツール定義の理由を `gbrain-contract.md` に記録した。既存の安全機構は削除対象にしていない。

検証: Go全テスト・race・vet、Web型チェック・テスト・buildが成功。Playwrightのブラウザ回帰（APIは模擬応答）と、使い捨てPostgreSQLの4つのrepository結合テストも成功。追加確認は認可context欠落・保存先不一致・管理操作へのwriter流用の拒否、読み取り範囲更新失敗時の転送停止、大きなJSON-RPC ID、接続状態と起動時復旧。

ローカルAPIを再ビルドし、health・OAuth discovery・未認証時の拒否とmigration `7 / clean` を確認。既存ユーザーのページに対する実書き込みは実施していない。変更前イメージ `brainhub-api:before-gbrain-boundary-5f78937` を保持した。
